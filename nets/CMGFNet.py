...
class CMGFNet(nn.Module):
    def __init__(self, n_classes=2, pretrained=False, is_deconve=False):
        super().__init__()
        #num_classes = 1
        ...
        # replace the first conv1 with a new conv1 that handles 1 channel data
        ...
        self.final_fused = nn.Sequential(
            nn.Conv2d(32, self.n_classes, kernel_size=1, padding=0),
            nn.BatchNorm2d(self.n_classes),
            nn.ReLU(inplace=True),
            )
        
        self.final_rgb = nn.Sequential(
            nn.Conv2d(16, self.n_classes, kernel_size=1, padding=0),
            nn.BatchNorm2d(self.n_classes),
            nn.ReLU(inplace=True),
            )
